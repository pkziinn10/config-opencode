...
After verdict, use `question` tool:
- NO-GO: "BLOCK found. (a) invoke spec to fix §B/§V now (b) show me the finding detail (c) override and build anyway"
- GO: "Review passed. (a) invoke build now (b) invoke spec to harden §V first (c) done"
